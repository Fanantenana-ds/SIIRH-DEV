...
def check_new_mails():
    ...
    try:
        ...
        for num in messages[0].split()[::-1]:
            ...
            # 🔹 Création candidature temporaire
            candidature = Candidature(
                fullname="INCONNU",   # soloina rehefa voavaky ny fichier
                nom="INCONNU",
                prenom="INCONNU",
                email=from_email,
                poste=offre.title,
            ...
            for part in msg.walk():
                if part.get_filename():
                    ...
                    if filename and filename.lower().endswith((".pdf", ".docx")):
                        ...
                        # 🔹 Maka Nom / Prénom avy amin'ny nom du fichier
                        ...
                        candidature.fullname = f"{prenom} {nom}".strip()
                        db.commit()


                        # 🔹 Parsing CV ho an'ny score
                        cv_info = process_cv_from_bytes(
                            db,
                            content,
                            filename,
                        ...
                        if cv_info:
                            exp_years = cv_info.get("exp_years", 0)
                            skills = cv_info.get("skills", [])
                            projects = cv_info.get("projects", [])
                            candidature.score = (
                                min(exp_years / 10, 1) * 5 +
                                min(len(skills) / 10, 1) * 3 +
                                min(len(projects) / 5, 1) * 2
                            )
                            db.commit()
                        print("📩 MAIL REÇU")
