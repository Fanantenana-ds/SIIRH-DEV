...
# ==========================================================
# 🔹 PUT update statut (AJOUTE CE CODE)
# ==========================================================
@router.put("/candidatures/{id}/update_statut")
async def update_candidature_statut(id: int, statut_data: dict):
    """
    Endpoint pour mettre à jour le statut d'une candidature
    """
    try:
        new_statut = statut_data.get("statut")
        ...
        with engine.begin() as conn:
            res = conn.execute(query, {"statut": new_statut, "id": id})
            
            if res.rowcount == 0:
                raise HTTPException(status_code=404, detail="Candidature non trouvée")
            
            print(f"✅ Statut candidature ID {id} mis à jour: {new_statut}")
        
        return {
            "message": f"Statut mis à jour avec succès: {new_statut}",
            "id": id,
            "statut": new_statut
        }
...
# 🔹 DELETE candidature
# ==========================================================
@router.delete("/candidatures/{id}")
async def delete_candidature(id: int):
    """
    Supprimer une candidature
    """
    try:
        print(f"🗑️ Tentative suppression candidature ID: {id}")
        
        # Vérifier d'abord si la candidature existe
        check_query = sqlalchemy.text("SELECT id FROM candidatures WHERE id = :id")
        
        with engine.begin() as conn:
            # Check
            result = conn.execute(check_query, {"id": id})
            candidature = result.fetchone()
            
            if not candidature:
                print(f"❌ Candidature ID {id} non trouvée")
                raise HTTPException(status_code=404, detail=f"Candidature ID {id} non trouvée")
            
            print(f"✅ Candidature ID {id} trouvée, suppression en cours...")
            
            # 1. Supprimer d'abord les convocations associées
            delete_convocations_query = sqlalchemy.text(
                "DELETE FROM convocations WHERE candidature_id = :id"
            )
            conv_result = conn.execute(delete_convocations_query, {"id": id})
            print(f"   📅 Convocations supprimées: {conv_result.rowcount}")
            
            # 2. Supprimer les notifications associées
            try:
                delete_notifs_query = sqlalchemy.text(
                    "DELETE FROM notifications WHERE candidature_id = :id"
                )
                notif_result = conn.execute(delete_notifs_query, {"id": id})
                print(f"   🔔 Notifications supprimées: {notif_result.rowcount}")
            except:
                print("   ℹ️ Pas de notifications à supprimer")
            
            # 3. Supprimer la candidature
            delete_candidature_query = sqlalchemy.text(
                "DELETE FROM candidatures WHERE id = :id"
            )
            candidature_result = conn.execute(delete_candidature_query, {"id": id})
            
            if candidature_result.rowcount == 0:
                raise HTTPException(status_code=404, detail="Candidature non trouvée")
            
            print(f"✅ Candidature ID {id} supprimée avec succès")
        
        return {
            "success": True,
            "message": f"Candidature ID {id} supprimée avec succès",
            "deleted_id": id
        }
...
# ==========================================================
# 🔹 RESET candidature (Convoqué → Sélectionné)
# ==========================================================
@router.put("/candidatures/{id}/reset")
async def reset_candidature(id: int):
    """
    Réinitialiser une candidature convoquée
    """
    try:
        print(f"🔄 Tentative reset candidature ID: {id}")
        
        check_query = sqlalchemy.text("""
            SELECT id, fullname, statut 
            FROM candidatures 
            WHERE id = :id
        """)
        ...
        with engine.begin() as conn:
            ...
            candidat_id, fullname, current_statut = row
            
            print(f"   👤 Candidat: {fullname}")
            print(f"   📊 Statut actuel: {current_statut}")
            
            # Vérifier si c'est un candidat convoqué
            if current_statut != "Convoqué":
                return {
                    "warning": True,
                    "message": f"Le candidat n'est pas 'Convoqué'. Statut actuel: {current_statut}",
                    "current_statut": current_statut,
                    "action": "Aucune"
                }
            
            # Supprimer les convocations associées
            delete_convocations_query = sqlalchemy.text(
                "DELETE FROM convocations WHERE candidature_id = :id"
            )
            conv_result = conn.execute(delete_convocations_query, {"id": id})
            ...
            update_query = sqlalchemy.text("""
                ...
                WHERE id = :id
            """)
            update_result = conn.execute(update_query, {"id": id})
            
            print(f"✅ Candidature ID {id} réinitialisée: Convoqué → Sélectionné")
        
        return {
            "success": True,
            "message": f"Candidature '{fullname}' réinitialisée: maintenant 'Sélectionné'",
            "id": id,
            "nom": fullname,
            "ancien_statut": current_statut,
            "nouveau_statut": "Sélectionné"
        }
...
# ==========================================================
@router.get("/candidatures/{id}")
async def get_candidature_by_id(id: int):
    """
    Obtenir une candidature spécifique par ID
    """
    try:
        query = sqlalchemy.text("""
            SELECT 
                id, fullname, email, phone, poste, statut,
                date_candidature, score, offre_id, ref_offre,
                source, created_at
            FROM candidatures 
            WHERE id = :id
        """)
        ...
        with engine.begin() as conn:
            ...
            # Convertir en dict
            candidature = dict(row._mapping)
            
            # Formater les dates
            if candidature.get("date_candidature"):
                candidature["date_candidature"] = candidature["date_candidature"].isoformat()
            
            if candidature.get("created_at"):
                candidature["created_at"] = candidature["created_at"].isoformat()
