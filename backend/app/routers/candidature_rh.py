...
# ==========================================================
# 🔹 PUT sélection / désélection (inchangé)
# ==========================================================
@router.put("/candidatures/{id}/select")
async def select_candidature(id: int):
    try:
...
# ==========================================================
# 🔹 POST convocation + mail automatique (inchangé)
# ==========================================================
@router.post("/candidatures/{id}/send-invitation")
async def send_invitation(id: int, db: Session = Depends(get_db)):
    try:
...
# ==========================================================
# 🔹 POST formulaire candidature + upload MinIO (nouveau)
# ==========================================================
@router.post("/api/candidatures")
async def create_candidature(
    ...
    poste: str = Form(...),
    cv: UploadFile = File(...),
    lm: UploadFile = File(None),
    diplomes: UploadFile = File(None),
    db: Session = Depends(get_db)
):
    try:
        print("DEBUG FORM FIELDS:", ref_offre, nom, prenom, email, poste, cv, lm, diplomes)

        # Vérification offre
        from app.models.offres import Offre
        offre = db.query(Offre).filter(Offre.titre == poste).first()
        if not offre:
        ...
        # Upload fichiers any MinIO
        upload_files_to_minio(db, new_candidature.id, [cv, lm, diplomes])
